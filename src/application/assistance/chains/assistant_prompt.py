...
class RequiredVariableMissingError(Exception):
    def __init__(self, variable):
        super().__init__(
            f"Required variable '{variable}' is not used in either the system or user template."
        )
...
class UserDefinedVariableMissingError(Exception):
    def __init__(self, variable):
        super().__init__(
            f"User-defined variable '{variable}' is not used in either the system or user template."
        )
...
class AssistantPromptBuilder:
    def __init__(self, system_template: str = None, user_template: str = None):
        ...
        self.__system_template = (
            system_template if system_template is not None else DEFAULT_SYSTEM_TEMPLATE
        )
        self.__user_template = (
            user_template if user_template is not None else DEFAULT_USER_TEMPLATE
        )
    ...
    def _validate(self):
        for variable in self.required_variables:
            ...
            if (
                wrapped_variable not in self.__system_template
                and wrapped_variable not in self.__user_template
            ):
    ...
    def _retrieve_prompt_from_file(self, filepath):
        ...
        try:
            ...
            raise IOError(
                f"An error occurred while reading the file '{filepath}': {str(e)}"
            )
