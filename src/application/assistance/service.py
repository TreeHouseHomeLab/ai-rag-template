...
class AssistantService:
    ...
    def __init__(
        ...
        self.configuration = configuration or AssistantServiceConfiguration(
    ...
    def _init_embeddings(self):
        ...
        embeddings_model = OpenAIEmbeddings(
            openai_api_key=embeddings_api_key, model=embeddings_config.name
        )
    ...
    def _init_retriever_chain(self, embeddings: Embeddings):
        ...
        db_name = vector_store_configurations.dbName or parse_uri(
            vector_store_cluster_uri
        ).get("database")

        if db_name is None:
            raise ValueError(
                "Database name is not provided in the configuration or the cluster URI"
            )
        ...
        retriever_chain = RetrieverChain(
            context=self.app_context, configuration=configuration
        )
        ...
        try:
            if self.app_context.configurations.chain.rag.promptsFilePath:
                ...
                if self.app_context.configurations.chain.rag.promptsFilePath.user:
                    builder.load_user_template_from_file(
                        self.app_context.configurations.chain.rag.promptsFilePath.user
                    )
    ...
    def chat_completion(
        ...
        with get_openai_callback() as openai_callback:
            ...
            if custom_template_variables:
                inputs[self._chain.prompt_custom_variables_key] = (
                    custom_template_variables
                )

            chain_response = self._chain.invoke(inputs)

            self.app_context.metrics_manager.requests_tokens_consumed.inc(
                openai_callback.prompt_tokens
            )
            self.app_context.metrics_manager.reply_tokens_consumed.inc(
