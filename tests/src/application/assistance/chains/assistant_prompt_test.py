...
def test_assistant_prompt_builder():
    ...
    assert (
        "You MUST also consider this new variable: {new_variable}."
        in assistant_prompt.system_template
    )
...
def test_missing_required_variable():
    builder = AssistantPromptBuilder(
        system_template="{chat_history}", user_template="{query}"
    )
    ...
    assert (
        str(excinfo.value)
        == "Required variable 'output_text' is not used in either the system or user template."
    )
