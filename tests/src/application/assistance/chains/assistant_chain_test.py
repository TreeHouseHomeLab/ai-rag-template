...
def test_call(mock_retreive_call, app_context, snapshot):
    ...
    llm = FakeLLM(
        sequential_responses=True, queries={"1": mock_response}
    )  # LLM service
    ...
    retriever_chain = RetrieverChain(
        context=app_context, configuration=vector_store_configuration
    )
    ...
    assert (
        chain_invoked[assistant_chain.references_key]
...
def test_call_without_documents(mock_retreive_call, app_context, snapshot):
    ...
    snapshot.assert_match(
        llm.get_last_received_prompt(), "last_received_prompt_without_documents"
    )
...
def test_call_with_custom_prompt(mock_retreive_call, app_context, snapshot):
    ...
    custom_prompt = (
        ...
        .append_to_system_template(
            "This is a custom prompt with a variable: {a_custom_variable}"
        )
        .build()
    )
    ...
    inputs = {
        assistant_chain.query_key: mock_query,
        assistant_chain.chat_history_key: mock_chat_history,
        assistant_chain.prompt_custom_variables_key: {
    ...
    # Assert that the last received prompt matches the stored snapshot
    snapshot.assert_match(
        llm.get_last_received_prompt(), "last_received_prompt_with_custom_variables"
    )
...
def test_call_without_chat_history(mock_retreive_call, app_context, snapshot):
    ...
    # Assert that the last received prompt matches the stored snapshot
    snapshot.assert_match(
        llm.get_last_received_prompt(), "last_received_prompt_without_chat_history"
    )
