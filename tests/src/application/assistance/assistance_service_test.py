...
def test_init_with_db_name_from_uri(app_context):
    try:
        ...
        modified_app_context.env_vars.MONGODB_CLUSTER_URI = (
...
def test_chat_completion(
    similarity_search_with_score, app_context, mock_server, snapshot
):
    # Arrange
    ...
    chat_completion = mock_server.respx_mock.post(
        "https://api.openai.com/v1/chat/completions"
    ).mock(return_value=Response(200, json=chat_completion_reply_mock))
    ...
    assert (
        result.response
        == chat_completion_reply_mock["choices"][0]["message"]["content"]
    )
...
def test_chat_completion_with_custom_template(
    ...
    assistant_service_config = AssistantServiceConfiguration(
        ...
        .append_to_system_template(
            "This is a custom template addition with a variable: {a_custom_variable}"
        )
    ...
    assistant_service = AssistantService(
        app_context=app_context, configuration=assistant_service_config
    )
...
def test_chat_completion_with_prompts_from_file(
    ...
    snapshot.assert_match(
        chat_completion.calls[0][0].content,
        "chat_completion_request_with_prompts_from_file",
    )
